...
const flashcard_create = async (req, res) => {
    try {
        ...
        if (!userId) {
            return res.status(401).json({ success: false, message: "Unauthorized: User ID missing" }); // user must be signed-in
        }

        console.log("User ID:", userId); // Log user ID (debugg log)

        let collection = await FlashcardCollection.findOne({ topic: topicSmall, userId });
...
const flashcard_view_topicname = async (req , res)=>{
    try {
        ...
        const collection = await FlashcardCollection.findOne({topic: topicSmall , userId});
        if (!collection) {
            return res.status(404).json({ success: false, message: "No flashcards found for this topic" });
        }

        const flashcards = collection.FlashcardsId;//array of flashcard ids 
        res.status(200).json({
            success: true,
            flashcards,
        });
...
const flashcard_edit = async (req, res) => {
    try {
        ...
        if (topicSmall) {
            ...
            newCollection = await FlashcardCollection.findOne({ topic: topicSmall, userId });
            console.log(newCollection);
            //if no such collection of that topic exists  make a new collection
            if (!newCollection) {
                newCollection = new FlashcardCollection({ userId: userId.toString(), topic: topicSmall, FlashcardsId: [] });
            ...
            if (String(flashcard.flashcardCollectionId) !== String(newCollection._id)) {
                const oldCollection = await FlashcardCollection.findById(flashcard.flashcardCollectionId);
                if (oldCollection) {
                    oldCollection.FlashcardsId = oldCollection.FlashcardsId.filter(id => id.toString() !== flashcardId);
        ...
        await flashcard.save();
        res.status(200).json({ success: true, message: "Flashcard updated successfully", flashcard });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
...
const flashcard_delete = async (req, res) => {
    try {
        ...
        const collection = await FlashcardCollection.findById(flashcard.flashcardCollectionId);
        if (!collection || collection.userId.toString() !== userId.toString()) {
            return res.status(403).json({ success: false, message: "Unauthorized: You cannot delete this flashcard" });
        }

        // remove flashcard from the collection's FlashcardsId array
        collection.FlashcardsId = collection.FlashcardsId.filter(id => id.toString() !== flashcardId);
        await collection.save();
