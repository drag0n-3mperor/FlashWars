...
const initiate_register = async (req, res) => {
    try {
      ...
      userStorage.set(email, { username, fullname, email, password, profileImage });
  ...
  const verify_otp_and_register = async (req, res) => {
    try {
      ...
      if (!userData) {
        return res.status(400).json({ message: "User data not found, please restart registration" });
      }
      ...
      console.log("File uploaded successfully on cloud. Cloud link:", uploadResponse.url);
      ...
      res.cookie("accessToken",accessToken,{
        httpOnly: true, // Prevent access to the cookie via JavaScript
      sameSite: "None", // Prevent CSRF by restricting cookie to same-site requests
...
export { initiate_register, verify_otp_and_register , user_login , refresh_access_token };
